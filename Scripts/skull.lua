		...
		if bullet then
			if bullet.alive + dt > skull.livetime or bullet.hit then --lives for 2 sec, clean up after
				...
				for k, v in pairs(sm.player.getAllPlayers()) do
					local targetdir = sm.vec3.normalize(bullet.pos - v.character.worldPosition)
					local yaw = (180 + math.atan(targetdir.y,targetdir.x)/math.pi * 180) - (180 + math.atan(bullet.direction.y,bullet.direction.x)/math.pi * 180)
				...
				if bullet.alive > 2 then
					bullet.direction = (bullet.direction*0.7 + sm.vec3.normalize(pos - bullet.pos)*(0.125*bullet.power + bullet.direction:length()*0.3) )*0.995
				else
...
function skull.client_onShoot( self, data )
	...
	local bullet = {
		effect = sm.effect.createEffect("fireskull"),
		pos = position,
		direction = data.dir,
		alive = 0,
		power = data.power,
